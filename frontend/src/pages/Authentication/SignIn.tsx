  ...
  const requestOptions = {
    ...
    body: JSON.stringify({
      ...
      if (result.status === 'success') {
        ...
        localStorage.setItem('userId', result.currentUserCode);
        result.role === 'admin' ? navigate('/') : navigate('/user');
      } else {
      ...
      <div className='flex flex-wrap items-center fixed inset-0 '>
        ...
        <div className='w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2'>
          <div className='w-full p-4 sm:p-12.5 xl:p-17.5'>
            <span className='mb-1.5 block font-medium'>See the Dashboard</span>
            ...
            <form onSubmit={handleSubmit}>
              <div className='mb-4'>
                ...
                <div className={`relative ${emailError ? 'border-red-500' : ''}`}>
                  <input
                    type='email'
                    placeholder='Enter your email'
                    className={`w-full rounded-lg border  bg-transparent py-4 pl-6 pr-10 text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary ${emailError ? 'border-red-500' : 'border-stroke'}`}
                    value={email}
                    onChange={(e) => {
              ...
              <div className='mb-6'>
                ...
                <div className={`relative ${passwordError ? 'border-red-500' : ''}`}>
                  <input
                    type={showPassword ? 'text' : 'password'} // Show text or password based on showPassword state
                  ...
                  <span className='absolute right-4 top-4'>
                    {showPassword ? (
                      ...
                      class="lucide lucide-eye-off" onClick={() => setShowPassword(true)} ><path d="M9.88 9.88a3 3 0 1 0 4.24 4.24"/><path d="M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 10 7 10 7a13.16 13.16 0 0 1-1.67 2.68"/><path d="M6.61 6.61A13.526 13.526 0 0 0 2 12s3 7 10 7a9.74 9.74 0 0 0 5.39-1.61"/><line x1="2" x2="22" y1="2" y2="22"/></svg>
                    )}
                  </span>
                </div>
              </div>
...
//       if (!response.ok) {
//         // If response status is not in the 200-299 range (e.g., 401 Unauthorized)
//         const errorText = await response.json(); // Get error message from response body
//         throw new Error(errorText.message); // Throw an error with the error message
//       }

//       const result = await response.json(); // Parse response body as JSON

//       // Log the message from the result (assuming result is an object with a message property)
//       if (result.status === 'success') {
...
//     } catch (err) {
//       console.log(err);
//       toast.error('Error: Incorrect Email or password'); // Log the entire error object for inspection
//       // Access specific properties of the error object if needed
//       // if (err instanceof Error) {
//       //   console.log('Error message:', err.message); // Log the error message from the Error object
//       // }
//     }finally {
//       setIsLoading(false);
//     }
...
//               <input
//                   type='submit'
//                   value={isLoading ? 'Fetching...' : 'Sign In'}
//                   className={`w-full cursor-pointer rounded-lg border border-primary bg-primary p-4 text-white transition hover:bg-opacity-90 ${
//                     isLoading ? 'opacity-50 pointer-events-none' : '' // Disable interactions when loading
//                   }`}
//                   disabled={isLoading}
//                 />
//               </div>
