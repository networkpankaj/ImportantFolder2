...
import LogoDark from '../../images/logo/Isure-media-logo.webp';
...
const SignUpUser: React.FC = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/v1/user/signup?token=${token}`,
        requestOptions
      );
      ...
      console.log(result);
      window.location(''); // Log the response text
    } catch (error) {
  ...
  return (
    <div className='rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark fixed inset-0'>
      <div className='flex flex-wrap items-center fixed inset-0'>
        ...
        <div className='w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2'>
          <div className='w-full p-4 sm:p-12.5 xl:p-17.5'>
            <span className='mb-1.5 block font-medium'>Admin Send </span>
            ...
            <div className='mb-6'>
                ...
                <div className={`relative ${passwordError ? 'border-red-500' : ''}`}>
                  <input
                    type={showPassword1 ? 'text' : 'password'} // Show text or password based on showPassword state
                    placeholder='8+ Characters, 1 Capital letter'
                    className={`w-full rounded-lg border  bg-transparent py-4 pl-6 pr-10 text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary ${passwordError ? 'border-red-500' : 'border-stroke'}`}
                    value={password}
                    onChange={(e) => {
                  ...
                  <span className='absolute right-4 top-4'>
                    {showPassword1 ? (
                      ...
                      class="lucide lucide-eye-off" onClick={() => setShowPassword1(true)} ><path d="M9.88 9.88a3 3 0 1 0 4.24 4.24"/><path d="M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 10 7 10 7a13.16 13.16 0 0 1-1.67 2.68"/><path d="M6.61 6.61A13.526 13.526 0 0 0 2 12s3 7 10 7a9.74 9.74 0 0 0 5.39-1.61"/><line x1="2" x2="22" y1="2" y2="22"/></svg>
                    )}
                  </span>
                </div>
              </div>
              ...
              <div className='mb-6'>
                ...
                <div className={`relative ${passwordError ? 'border-red-500' : ''}`}>
                  <input
                    type={showPassword2 ? 'text' : 'password'} // Show text or password based on showPassword state
                    placeholder='8+ Characters, 1 Capital letter'
                    className={`w-full rounded-lg border  bg-transparent py-4 pl-6 pr-10 text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary ${passwordError ? 'border-red-500' : 'border-stroke'}`}
                    value={confirmPss}
                    onChange={(e) => {
                  ...
                  <span className='absolute right-4 top-4'>
                    {showPassword2 ? (
                      ...
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" 
                      class="lucide lucide-eye-off" onClick={() => setShowPassword2(true)} ><path d="M9.88 9.88a3 3 0 1 0 4.24 4.24"/><path d="M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 10 7 10 7a13.16 13.16 0 0 1-1.67 2.68"/><path d="M6.61 6.61A13.526 13.526 0 0 0 2 12s3 7 10 7a9.74 9.74 0 0 0 5.39-1.61"/><line x1="2" x2="22" y1="2" y2="22"/></svg>
                    )}
                  </span>
                </div>
              </div>
