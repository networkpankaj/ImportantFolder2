...
const Modal = ({ onClose, isEdit, requestId, updateRequestData, fetchData }) => {
...
//   
  ...
  const fetchRequest = async (id) => {
    try {
      ...
      setRequestName(result.data.request);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to ${isEdit ? 'update' : 'add'} request`);
      }
      const data = await response.json();
      toast(`${isEdit ? 'Updated' : 'Added'} request successfully`, data);
      updateRequestData(data);
      fetchData();
      onClose();
    } catch (error) {
      toast.error(`Error ${isEdit ? 'updating' : 'adding'} request:`, error);
    }
  };
          ...
          <div className='bg-indigo-600 rounded-xl px-20 py-10 flex flex-col gap-5 items-center mx-4'>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className='w-full sm:w-1/2'>
                  <div className='relative'>
                    ...
                    <input
                      ...
                      value={getDateFromISOString(requestName.date)}
              ...
              <textarea
                ...
                required
                className='w-full px-4 py-3 text-black border-gray-300 rounded-md'
                value={requestName.reason}
                onChange={(e) => setRequestReason(e.target.value)}></textarea>
